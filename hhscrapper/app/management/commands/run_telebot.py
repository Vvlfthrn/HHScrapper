...
async def bot_do_work():
    ...
    if await query.aexists():
        ...
        vacancies = [x async for x in query.all()]
        ...
        await Vacancy.objects.filter(id__in=[x.id for x in vacancies]).aupdate(notified=True)
